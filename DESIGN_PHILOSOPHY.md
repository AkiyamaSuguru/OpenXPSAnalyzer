# XPS Analyzer 软件设计哲学手册

> 产品与技术设计原则 · Version 1.0
> 适用于 XPS Analyzer 0.3.x 及后续兼容版本
> Copyright © 2026 Jay Mamun. All rights reserved.

## 1. 手册目的

这份手册说明 XPS Analyzer 为什么以当前方式工作，以及后续功能应当如何被设计、评审和实现。它不是逐个按钮的使用教程，也不是某个框架的编程指南，而是项目在产品体验、科学数据、软件架构和长期维护方面的共同判断标准。

当新需求、技术便利或视觉偏好彼此冲突时，应优先遵守本手册中的核心原则。若确实需要偏离，应记录原因、影响范围、迁移方案和验证证据。

## 2. 产品使命

XPS Analyzer 的使命是把 Avantage 导出的 XPS 数据转化为一个可信、清晰、可追溯、可复用的分析项目，同时让研究者把注意力放在谱图和科学判断上，而不是文件格式、绘图库或界面操作上。
...
XPS Analyzer 的产品署名、构建元数据、软件说明、README 和许可证必须保持一致：

```text
Copyright © 2026 Jay Mamun. All rights reserved.
```

本项目为专有软件，软件及源代码著作权归 Jay Mamun 所有。功能贡献、第三方依赖或自动生成内容不得擅自改变产品署名和许可方式。第三方依赖仍分别遵守其自身许可证；发布前应完成依赖许可证审查。

本手册描述产品设计原则，具体授权边界以项目根目录的 `LICENSE` 为准。
